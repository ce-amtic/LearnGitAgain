...
enable CrashBlockAimed system     [dev]
  ...
  2. KeyBit SlightAjustment system under inspection  [pending]
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
LinkNode * Mutate(LinkNode *target){
  ...
  newinfo->mark = 0, newinfo->next = NULL;
  ...
  if(CurMode == 0){
    ...
    if(tarinfo->mark){ // 对关键字段进行微调
      for(int i = 0; i < 29; i++){
        switch(rand() % 3){
          case 0:
            buffer[KeyBit[i]] = (buffer[KeyBit[i]] + 1) % 256;
            break;
          case 1:
            buffer[KeyBit[i]] = (buffer[KeyBit[i]] + 255) % 256;
            break;
          case 2:
            //do nothing 
        }
